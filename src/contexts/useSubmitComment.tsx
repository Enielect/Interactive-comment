...
import { CommentContext } from "./CommentData";
...
function useSubmitComment(parentObject: Comment, comment: Comment, action: string) {
  const { data } = useContext(CommentContext);
  const { userData, comments } = data;

  const highestId =
    comments && Math.max(...comments.map(findHighestIdInObject));
